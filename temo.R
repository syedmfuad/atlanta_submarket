...
X <- cbind(1, I(log(data$calcacres)), I(data$age), 
           I(data$age*data$age)/1000, I(data$totbath), I(data$rmtot), I(data$rmbed), I(data$fixtot), I(data$fixhalf), I(data$fixaddl), I(data$fourthquart),
           I(data$sqft)/1000, I(data$tract_cover), I(data$total_crime_house), I(data$pct_above_f), I(data$pct_below_f),
           I(data$MATHMeanScaleScore15), I(data$pct_HSdiploma), I(data$pct_black), I(data$log_median_income),
...
Z <- cbind(1,I(data$pct_black), I(data$pct_renter_occupied), I(data$ENGMeanScaleScore15),
...
X <- cbind(1, I(log(data1$calcacres)), I(data1$age), 
           I(data1$age*data1$age)/1000, I(data1$totbath), I(data1$rmtot), I(data1$rmbed), I(data1$fixtot), I(data1$fixhalf), I(data1$fixaddl), I(data1$fourthquart),
           I(data1$sqft)/1000, I(data1$tract_cover), I(data1$total_crime_house), I(data1$pct_above_f), I(data1$pct_below_f),
           I(data1$MATHMeanScaleScore15), I(data1$pct_HSdiploma), I(data1$pct_black), I(data1$log_median_income),
...
X <- cbind(1, I(log(data2$calcacres)), I(data2$age), 
           I(data2$age*data2$age)/1000, I(data2$totbath), I(data2$rmtot), I(data2$rmbed), I(data2$fixtot), I(data2$fixhalf), I(data2$fixaddl), I(data2$fourthquart),
           I(data2$sqft)/1000, I(data2$tract_cover), I(data2$total_crime_house), I(data2$pct_above_f), I(data2$pct_below_f),
           I(data2$MATHMeanScaleScore15), I(data2$pct_HSdiploma), I(data2$pct_black), I(data2$log_median_income),
...
X <- cbind(1, I(log(data$calcacres)), I(log((data$median_income)/10000)), I(data$age), 
           ...
           I(data$pct_black), I(data$total_crime_house))
...
reg <- lm(salesprice ~ calcacres + age + I(age*age) + totbath + rmtot + rmbed + fixtot + fixhalf + fixaddl + 
            fourthquart + I(sqft) + I(lon-plon) + I(lat-plat) + I((lon-plon)^2) + I((lat-plat)^2) + 
            as.factor(dcdu), data=data)
...
reg <- lm(salesprice ~ calcacres + age + I(age*age) + totbath + rmtot + rmbed + fixtot + fixhalf + fixaddl + stories + 
            centheat + fourthquart + I(sqft) + I(lon-plon) + I(lat-plat) + I((lon-plon)^2) + I((lat-plat)^2) + 
            as.factor(revcode) + as.factor(saleval) + as.factor(saletype) + 
            as.factor(saledtyearyyyy) + as.factor(dcdu), data=data)

summary(reg)

deviance(reg)
pred <- predict(reg)
c(crossprod(reg$residuals)) #rss
c(crossprod(reg$residuals))/length(reg$residuals) #mse
sqrt(c(crossprod(reg$residuals))/length(reg$residuals)) #rmse

median_rooms pct_above_f	pct_below_f


reg <- lm(salesprice ~ calcacres + age + I(age*age) + totbath + rmtot + rmbed + fixtot + fixhalf + fixaddl + stories + 
            centheat + fourthquart + I(sqft) + I(lon-plon) + I(lat-plat) + I((lon-plon)^2) + I((lat-plat)^2) + 
            I(pct_black) + as.factor(revcode) + as.factor(deedtype) + as.factor(saleval) + as.factor(saletype) + as.factor(saledtmonthmon) + 
            as.factor(saledtyearyyyy) + years_since_remodel + as.factor(extwall) + as.factor(style) + as.factor(fuel) + as.factor(attic) + 
            as.factor(heatsys) + as.factor(fronting) + as.factor(street1) + as.factor(topo1) + as.factor(util1) + as.factor(parktype) + 
            as.factor(parkprox) + as.factor(dcdu) + as.factor(location) + as.factor(class) + as.factor(luc) + as.factor(zoning), data=data)
